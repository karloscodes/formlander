...
var (
	NetworkName      = productSettings.NetworkName
	CaddyName        = productSettings.CaddyContainer
	AppNamePrimary   = productSettings.AppPrimaryContainer
	AppNameSecondary = productSettings.AppSecondaryContainer
)
...
func (d *Docker) DeployApp(data config.ConfigData, name string) error {
	...
	envVars := []string{
		"-e", fmt.Sprintf("%s=debug", productSettings.EnvKey("LOG_LEVEL")),
		"-e", fmt.Sprintf("%s=8080", productSettings.EnvKey("APP_PORT")),
		"-e", fmt.Sprintf("%s=%s", productSettings.DomainEnvKey(), data.Domain),
		"-e", fmt.Sprintf("%s=%s", productSettings.PrivateKeyEnvKey(), data.PrivateKey),
		"-e", "SERVER_INSTANCE_ID=" + name,
	}
	// Always set license key env var for consistency
	envVars = append(envVars, "-e", fmt.Sprintf("%s=%s", productSettings.LicenseKeyEnvKey(), data.LicenseKey))
...
// VerifyContainersRunning checks if the application containers are running
func (d *Docker) VerifyContainersRunning() (bool, error) {
	// Check app container
	appRunning, err := d.isContainerRunning(productSettings.AppBaseContainer())
	if err != nil {
		return false, fmt.Errorf("failed to check app container: %w", err)
	}

	// Check Caddy container
	caddyRunning, err := d.isContainerRunning(CaddyName)
	if err != nil {
...
// Format: {admin_email_prefix}@{base_domain}
func generateAdminEmail(domain string) string {
	baseDomain := extractBaseDomain(domain)
	return fmt.Sprintf("%s@%s", productSettings.AdminEmailPrefix, baseDomain)
}
