...
func DefaultConfig() Config {
	return Config{
		...
		VMName:  fmt.Sprintf("%s-test-vm", productSettings.Slug),
...
func (r *TestRunner) runInVM() error {
	...
	tempBinaryPath := fmt.Sprintf("/home/ubuntu/%s", productSettings.BinaryName)
	copyCmd := exec.Command("multipass", "transfer", r.Config.BinaryPath, fmt.Sprintf("%s:%s", r.Config.VMName, tempBinaryPath))
	...
	execCmd := exec.Command("multipass", "exec", r.Config.VMName, "--", "chmod", "+x", tempBinaryPath)
	...
	moveCmd := exec.Command("multipass", "exec", r.Config.VMName, "--", "sudo", "mv", tempBinaryPath, productSettings.BinaryPath)
	...
	cmdParts := []string{"exec", r.Config.VMName, "--", "sudo", productSettings.BinaryPath}
