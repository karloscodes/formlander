...
import (
	...
	"formlander/internal/installer/requirements"
)
...
func (i *Installer) GetMainDBPath() string {
	...
	return filepath.Join(data.InstallDir, "storage", productSettings.DatabaseFileName)
}
...
func (i *Installer) DisplayCompletionMessage() {
	...
	if i.config.HasDNSWarnings() {
		...
		fmt.Printf("   4. Monitor logs: sudo tail -f %s\n", filepath.Join(productSettings.InstallDir, "logs", "caddy.log"))
	...
	_ = fmt.Sprintf("%s@%s", productSettings.AdminEmailPrefix, baseDomain) // Keep for potential future use
	fmt.Println()
	fmt.Printf("🚀 Your %s installation is ready!\n", productSettings.ProductName)
	fmt.Printf("Thank you for choosing %s.\n", productSettings.ProductName)
}
...
func (i *Installer) Run() error {
	...
	i.logger.Info("Step 4/%d: Configuring %s", totalSteps, productSettings.ProductName)
	// Step 4: Config
	data := i.config.GetData()
	if err := i.createInstallDir(data.InstallDir); err != nil {
		return fmt.Errorf("failed to create install dir: %w", err)
	...
	i.logger.Info("Step 5/%d: Deploying %s", totalSteps, productSettings.ProductName)
	// Step 5: Deploy
