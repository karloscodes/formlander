...
import (
	...
	"formlander/internal/installer/errors"
...
type ConfigData struct {
	...
	AppImage     string   // GitHub Release/Default: e.g., "karloscodes/app:latest"
	CaddyImage   string   // GitHub Release/Default: e.g., "caddy:2.9-alpine"
	InstallDir   string   // Default installation dir
	BackupPath   string   // Default: SQLite backup location
	PrivateKey   string   // Generated: secure random key stored in the installer .env file
	Version      string   // GitHub Release: Version of the installer binary (optional)
	InstallerURL string   // GitHub Release: URL to download new installer binary
...
func NewConfig(logger *logging.Logger) *Config {
	...
	return &Config{
		...
		data: ConfigData{
			...
			AppImage:     prod.AppImage,
			CaddyImage:   prod.CaddyImage,
			InstallDir:   prod.InstallDir,
			BackupPath:   filepath.Join(prod.InstallDir, "storage", "backups"),
			PrivateKey:   "",
			Version:      "latest",
			InstallerURL: prod.InstallerURL(),
		},
	}
}
...
func (c *Config) CollectFromUser(reader *bufio.Reader) error {
	...
	c.data.Domain = ""
	c.data.InstallDir = c.product.InstallDir

	// Collect domain
	for {
		fmt.Printf("Enter your domain name (e.g., %s): ", c.product.DomainExample)
...
func (c *Config) collectFromEnvironment() error {
	...
	// Read domain from environment (product-specific key first, then legacy DOMAIN)
	domain := os.Getenv(c.product.DomainEnvKey())
	if domain == "" {
		domain = os.Getenv("DOMAIN")
	}
	if domain == "" {
		return fmt.Errorf("%s (or DOMAIN) environment variable is required in non-interactive mode", c.product.DomainEnvKey())
	}
	...
	c.data.InstallDir = c.product.InstallDir
	c.data.BackupPath = filepath.Join(c.data.InstallDir, "storage", "backups")
	c.data.AppImage = c.product.AppImage
	c.data.CaddyImage = c.product.CaddyImage
...
func (c *Config) LoadFromFile(filename string) error {
	...
	for scanner.Scan() {
		...
		case c.product.PrivateKeyEnvKey():
			c.data.PrivateKey = value
		case c.product.UserEnvKey():
			c.data.User = value
		case c.product.LicenseKeyEnvKey():
	...
	if c.data.PrivateKey == "" {
		...
		if ferr == nil {
		fmt.Fprintf(f, "%s=%s\n", c.product.PrivateKeyEnvKey(), pk)
		f.Close()
		c.logger.Info("Added missing %s to %s", c.product.PrivateKeyEnvKey(), filename)
	}
	}
...
func (c *Config) SaveToFile(filename string) error {
	...
	if c.data.PrivateKey == "" {
		...
		c.data.PrivateKey = pk
		c.logger.Info("Generated new %s", c.product.PrivateKeyEnvKey())
	}
	...
	fmt.Fprintf(file, "%s=%s\n", c.product.DomainEnvKey(), c.data.Domain)
	...
	fmt.Fprintf(file, "%s=%s\n", c.product.PrivateKeyEnvKey(), c.data.PrivateKey)
	if c.data.User != "" {
		fmt.Fprintf(file, "%s=%s\n", c.product.UserEnvKey(), c.data.User)
	}
	if c.data.LicenseKey != "" {
		fmt.Fprintf(file, "%s=%s\n", c.product.LicenseKeyEnvKey(), c.data.LicenseKey)
	}
...
func (c *Config) GetMainDBPath() string {
	return filepath.Join(c.data.InstallDir, "storage", c.product.DatabaseFileName)
}
...
func (c *Config) FetchFromServer(_ string) error {
	...
	// Try new naming pattern first (product-specific)
	binaryNameNew := fmt.Sprintf("%s-v%s-%s", c.product.ReleaseBinaryPrefix, version, runtime.GOARCH)
	// Fallback to old naming pattern for backwards compatibility
	binaryNameOld := fmt.Sprintf("%s-v%s-%s", c.product.LegacyBinaryPrefix, version, runtime.GOARCH)
