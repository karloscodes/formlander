...
import (
	...
	"formlander/internal/installer/cron"
	"formlander/internal/installer/database"
	"formlander/internal/installer/docker"
...
var (
	gitHubRepo        = productSettings.GitHubRepo
	gitHubAPIURL      = fmt.Sprintf("https://api.github.com/repos/%s/releases/latest", gitHubRepo)
	BinaryInstallPath = productSettings.BinaryPath // Standard installation path
)
...
func NewUpdater(logger *logging.Logger) *Updater {
	fileLogger := logging.NewFileLogger(logging.Config{
		...
		LogDir:  productSettings.LogDir,
		LogFile: productSettings.UpdaterLogFile,
	})
	...
	if latestVersion != "" {
		if compareVersions(currentVersion, latestVersion) < 0 {
			...
			if downloadURL == "" {
				...
				if downloadURL == "" || downloadURL == fmt.Sprintf("https://github.com/%s/releases/latest", gitHubRepo) {
					// Try new naming pattern first
					downloadURL = fmt.Sprintf("https://github.com/%s/releases/download/v%s/%s-v%s-%s", gitHubRepo, latestVersion, productSettings.ReleaseBinaryPrefix, latestVersion, arch)
					...
					if err != nil || resp.StatusCode != http.StatusOK {
						...
						downloadURL = fmt.Sprintf("https://github.com/%s/releases/download/v%s/%s-v%s-%s", gitHubRepo, latestVersion, productSettings.LegacyBinaryPrefix, latestVersion, arch)
...
func (u *Updater) getLatestVersionAndBinaryURL() (string, string, error) {
	...
	arch := runtime.GOARCH
	// Try new naming pattern first
	expectedAssetNew := fmt.Sprintf("%s-v%s-%s", productSettings.ReleaseBinaryPrefix, latestVersion, arch)
	// Fallback to old naming pattern for backwards compatibility
	expectedAssetOld := fmt.Sprintf("%s-v%s-%s", productSettings.LegacyBinaryPrefix, latestVersion, arch)
...
func (u *Updater) updateBinary(url, binaryPath string) error {
	...
	newBinary := filepath.Join("/tmp", "formlander.new")
...
func extractVersionFromURL(url string) string {
	...
	for i, part := range parts {
		if i < len(parts) {
			filename := part
			// Try new naming pattern first (formlander-installer)
			if strings.HasPrefix(filename, "formlander-installer-v") {
				version := strings.TrimPrefix(filename, "formlander-installer-v")
			...
			if strings.HasPrefix(filename, "formlander-v") {
				version := strings.TrimPrefix(filename, "formlander-v")
				version = strings.TrimSuffix(version, "-amd64")
				version = strings.TrimSuffix(version, "-arm64")
				return version
			}
