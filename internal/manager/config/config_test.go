...
func TestSettersAndGetters(t *testing.T) {
	...
	expectedPath := fmt.Sprintf("/foo/bar/storage/%s", productSettings.DatabaseFileName)
	if got := c.GetMainDBPath(); got != expectedPath {
		t.Errorf("GetMainDBPath() = %q, want %q", got, expectedPath)
	}
...
func TestNewConfig_Defaults(t *testing.T) {
	c := NewConfig(testLogger(t))
	data := c.data
	if data.AppImage != productSettings.AppImage {
		t.Errorf("AppImage default = %q, want %q", data.AppImage, productSettings.AppImage)
	}
	if data.CaddyImage != "caddy:2.9-alpine" {
		t.Errorf("CaddyImage default = %q, want %q", data.CaddyImage, "caddy:2.9-alpine")
	}
	if data.InstallDir != productSettings.InstallDir {
		t.Errorf("InstallDir default = %q, want %q", data.InstallDir, productSettings.InstallDir)
	}
}
...
func TestLoadFromFile(t *testing.T) {
	...
	t.Run("ValidFile", func(t *testing.T) {
		...
		content := fmt.Sprintf(`%s=test.example.com
...
%s=testprivatekey123
`, productSettings.DomainEnvKey(), productSettings.PrivateKeyEnvKey())
...
func TestSaveToFile(t *testing.T) {
	...
	expected := []string{
		fmt.Sprintf("%s=save.example.com", productSettings.DomainEnvKey()),
		...
		fmt.Sprintf("%s=", productSettings.PrivateKeyEnvKey()), // Should be generated
	}
	...
	if !strings.Contains(contentStr, fmt.Sprintf("%s=", productSettings.PrivateKeyEnvKey())) {
		t.Errorf("SaveToFile() should include %s", productSettings.PrivateKeyEnvKey())
	}
}
...
func TestCollectFromEnvironment(t *testing.T) {
	...
	envVars := map[string]string{
		"NONINTERACTIVE":                  "1",
		productSettings.DomainEnvKey():    "env.example.com",
	...
	os.Unsetenv("DOMAIN")
	os.Unsetenv(productSettings.DomainEnvKey())
	err = c.collectFromEnvironment()
	if err == nil {
...
func TestConfigurationValidation(t *testing.T) {
	t.Run("ValidateCompleteConfiguration", func(t *testing.T) {
		...
		c.data.AppImage = productSettings.AppImage
		c.data.CaddyImage = "caddy:2.9-alpine"
		c.data.InstallDir = productSettings.InstallDir
		c.data.BackupPath = filepath.Join(productSettings.InstallDir, "backup")
		c.data.PrivateKey = "this-is-a-very-long-private-key-that-meets-minimum-requirements"
		c.data.Version = "v1.0.0"
...
func TestConfigurationDefaults(t *testing.T) {
	t.Run("NewConfigurationSetsDefaults", func(t *testing.T) {
		...
		expectedDefaults := map[string]string{
			"AppImage":   productSettings.AppImage,
			"CaddyImage": "caddy:2.9-alpine",
			"InstallDir": productSettings.InstallDir,
		}
