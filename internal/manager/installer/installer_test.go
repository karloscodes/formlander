...
func TestGetMainDBPath(t *testing.T) {
	...
	expectedPath := filepath.Join(DefaultInstallDir, "storage", prod.DatabaseFileName)
	...
	dbPath = installer.GetMainDBPath()
	expectedPath = filepath.Join("/custom/install/dir", "storage", prod.DatabaseFileName)
	assert.Equal(t, expectedPath, dbPath)
}
...
func TestConstants(t *testing.T) {
	prod := product.Get()
	assert.Equal(t, prod.InstallDir, DefaultInstallDir)
	assert.Equal(t, prod.BinaryPath, DefaultBinaryPath)
	assert.Equal(t, prod.CronFile, DefaultCronFile)
	assert.Equal(t, prod.CronSchedule, DefaultCronSchedule)
}
